...
class OUNoise:
    """Ornstein-Uhlenbeck噪声，用于确定性策略的探索"""
    ...
    def select_action(self, state, explore=False):
        ...
        with torch.no_grad():
            ...
            if explore:
                ...
                action = action + noise.unsqueeze(0)
...
class MASAC:
    def __init__(self, env, device="cpu", actor_lr=3e-4, critic_lr=3e-4, alpha_lr=3e-4, 
        ...
        """
        确定性策略版本的分布式多智能体AC (Distributed Deterministic AC)
        ...
        Args:
            ...
            alpha_lr: 温度参数学习率 (在确定性版本中不使用，但保留参数便于兼容)
            hidden_dim: 网络隐藏层维度
            gamma: 折扣因子
            tau: 软更新系数
    ...
    def take_action(self, states, explore=True):
        ...
        for i, agent in enumerate(self.agents):
            action = agent.select_action(states[i], explore=explore)
        ...
        try:
            ...
            with torch.no_grad():
                for i, agent in enumerate(self.agents):
                    ...
                    next_action = agent.actor(next_state)
            ...
            for i, agent in enumerate(self.agents):
                # 只使用自身观察更新策略
                _, actor_loss = agent.update_actor(
                    joint_states,
                    joint_actions,
                    states_tensor[i]
                )
    ...
    def train(self, num_episodes, max_steps, batch_size, update_interval=1, eval_interval=1000, render=False):
        ...
        for episode in range(num_episodes):
            ...
            for step in range(max_steps):
                ...
                # 更新状态和累积奖励 - 修改为sum形式
                states = next_states
                episode_reward += np.sum(rewards)  # 修改为求和而非平均
