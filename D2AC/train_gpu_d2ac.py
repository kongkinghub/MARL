...
def plot_results(rewards, eval_rewards=None, window=10, save=True, show=False):
    ...
    # 添加额外信息
    plt.figtext(0.01, 0.01, f"训练日期: {datetime.now().strftime('%Y-%m-%d %H:%M')} | ρ=0.4 | 确定性策略 | 并行训练", fontsize=8)
...
def test_agent(masac, env_name, render_delay=0.2):
    ...
    for episode in range(num_test_episodes):
        ...
        for step in range(max_steps):
            # 获取动作
            actions = masac.take_action(states, explore=False)  # 测试时不使用探索
